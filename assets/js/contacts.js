...
async function logoutUser() {
  try {
    ...
    if (logoutResponse.ok) {
      ...
      console.error(
        `Fehler beim Ausloggen. Statuscode: ${logoutResponse.status}`
      );
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (responseData.data) {
        ...
        throw new Error(
          `Konnte Daten mit dem Schlüssel "${key}" nicht finden.`
        );
      }
    } else {
      console.error(
...
function generateUserListSections(sortedContacts) {
  ...
  sortedContacts.forEach((userData, index) => {
    ...
    userListSections.push(
      generateUserEntryHTML(index, userData, nameInitials, randomColor)
    );
  });
...
function generateUserDetailsHTML(user, userId) {
  ...
  const nameInitials = userToEdit[0].name
   ...
   return /*html*/ `
        ...
        <h3>Email</h3>
        <a class="a" href="mailto:${userToEdit[0].email}">${userToEdit[0].email}</a>
        <h3>Telefon</h3>
        <b>+49${userToEdit[0].phone}</b>
...
function createContact() {
  ...
  if (
    ...
    displayValidationMessage(
      "Bitte überprüfen Sie Ihre Eingaben. Stellen Sie sicher, dass der Name nicht leer ist, die E-Mail-Adresse korrekt ist und die Telefonnummer gültig ist."
    );
    return false;
  }
...
function generateContactsHTML(contacts) {
  ...
  for (const contact of contacts) {
    ...
    const nameInitials = name
      ...
      .map((word) => word[0])
      .join("");
    userDetailsHTML += generateContactHTML(
      contact.color,
      nameInitials,
      name,
      email,
...
function deleteUser(userId) {
  ...
  const selectedUserEmail =
    sortContactsByName(contacts)[selectedUserIndex].email;
  const sortedContacts = contacts
    .slice()
    .sort((a, b) => a.name.localeCompare(b.name));
  const sortedIndex = sortedContacts.findIndex(
    (contact) => contact.email === selectedUserEmail
  );
...
function updateContact() {
  ...
  if (
    ...
    selectedUserIndex < 0 ||
    selectedUserIndex >= contacts.length
  ) {
    console.error("Benutzer nicht gefunden");
    return;
  }
