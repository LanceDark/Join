...
async function checkEmailExistence(email) {
    try {
        ...
        const userWithEmail = usersData.find(user => user.email === email);
        if (userWithEmail) {
            alert('Die E-Mail-Adresse wurde gefunden. Das Passwort wird zurückgesetzt.');
             resetPassword(email);
        } else {
...
async function changePassword() {
    ...
    if (!isValidEmail(email)) {
        ...
        return;}
    try {if (localStorage.getItem(email)) {
            alert('Diese E-Mail-Adresse wurde bereits verwendet. Bitte verwenden Sie eine andere.');
            return;}
        localStorage.setItem(email, newPassword);
        alert('Ihr neues Passwort wurde erfolgreich gespeichert.');
        window.location.href = '../../login.html';
    } catch (e) {
        console.error('Fehler beim Speichern des neuen Passworts im Local Storage:', e);}
}
